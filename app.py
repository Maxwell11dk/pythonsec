...
def info():
    ...
    if request.method == "POST":
        ...
        print("email" + email)
        email_statement = """SELECT * FROM slaves WHERE email = '%s';""" % email
        c.execute(email_statement)
        res = c.fetchall()
        data = res
        print(res)
        redirect("/infoConfirm")
        statement = """INSERT INTO slaves(id,email) VALUES(null,'%s');""" % (email,)
        print(statement)
        c.execute(statement)
        session["email"] = email_statement
        sent = True
    if not sent:
        return render_template("info.html")
    else:
        return redirect(url_for("infoConfirm"))


@app.route("/infoConfirm", methods=["GET"])
def infoConfirm():
    email = session["email"]
    print(email)
    db = connect_db()
    ...
    return render_template("infoConfirm.html", value=res)
    ...
    if request.method == "POST":
        if request.form["submit_button"] == "add note":
            ...
            if len(result) > 0:
                row = result[0]
                statement = (
                    """INSERT INTO notes(id,assocUser,dateWritten,note,publicID) VALUES(null,%s,'%s','%s',%s);"""
